...
sys.path.append(fr"{Path(__file__).resolve().parent.parent}/disposeCFG");
print("main:", sys.path);
print(sys.path);
...
def getCFG(code: str):
    ...
    answer = disposeAndGetResult(test.to_dict());
    return answer;



def disposeAndGetResult(cfg):
    """处理cfgdata,返回格式完整的cfg"""
    # json_cfgdata = json.dumps(cfg, indent=4);
    return cfg;
